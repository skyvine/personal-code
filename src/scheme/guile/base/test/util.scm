...
; # Documentation
; Miscellaneous artifacts that are useful when writing tests.
...
(define-module (skyler test util)
	...
	#:export (
		example-instances
		; Signature: (example-instances class default-value-psec)
		;
		...
		;
		; default-value-spec: A hashtable with symbols as keys and any kind of value as
		;                     elements. The table must contain an entry whose key is the slot
		;                     name.
		;
		; # Returns
		; The list of example instances. For example, let's say you have a class for storing
		; cross-stitch patterns. The class might be defined like this:
		;
		; ```scm
		...
		; ```
		;
		; All patterns must have a name and a size, but description and quality might be
		; default initialized. You want to make sure that your code works whether the optional
		; values have default data or realistic user data. This is particularly relevant for
		; quality, as the default value is a boolean but a user-supplied value would be an
		; integer.
		;
		; This function allows you to easily generate a list of instances which represent all
		; possible combinations of the optional slots being initialized or not. So if you have
		; a function `run-test-suite` which takes in an instance and tests it, you could do
		; this:
		;
		; ```scm
		...
		; with the following cases. Note that order is undefined by the API, and the
		; optionality of slots are determined automatically through the class-conventions
		; module.
		...
		print-all-fields
		; Signature: (print-all-fields obj port optional: (oneline? #f))
		;
		...
		;
		; port: The port that should be written to
		;
		; oneline: True if the output should contain no newlines, false if newlines may be
		;          used for ergonomic formatting.
))
